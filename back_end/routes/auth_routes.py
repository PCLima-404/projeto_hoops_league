...
from models.models import Jogador,Competicao, Participacao, StatusComp, RespostaParticipacao
...
async def criar_conta(usuario_schema:UsuarioSchema,session: Session=Depends(pegar_sessao)):
    if session.query(Jogador).filter(Jogador.email == usuario_schema.email).first():
        raise HTTPException(status_code=400, detail="Email já cadastrado")
    if session.query(Jogador).filter(Jogador.user == usuario_schema.user).first():
...
async def editar_usuario(atualizar_usuario:EditarUsuarioSchema,session:Session=Depends(pegar_sessao),usuario:Jogador=Depends(verificar_token)):
    ...
    if atualizar_usuario.nome is not None:
        usuario.nome = atualizar_usuario.nome
    if atualizar_usuario.idade is not None:
        usuario.idade = atualizar_usuario.idade
    if atualizar_usuario.email is not None:
        usuario.email = atualizar_usuario.email
    if atualizar_usuario.altura is not None:
        usuario.altura = atualizar_usuario.altura
    if atualizar_usuario.posicao_preferida is not None:
        usuario.posicao_preferida = atualizar_usuario.posicao_preferida
    if atualizar_usuario.senha is not None:
        usuario.senha = bcrypy_context.hash(atualizar_usuario.senha)
...
async def delete_user(session: Session = Depends(pegar_sessao), usuario: Jogador = Depends(verificar_token)):
    ...
    try:
        ...
        session.commit()
        session.refresh(usuario)
        return {"mensagem": "Perfil apagado com sucesso"}
    except Exception as e:
        session.rollback()
        raise HTTPException(status_code=500, detail="Erro ao apagar perfil")
